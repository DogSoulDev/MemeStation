...
router.put("/favourites/:userId", async (req, res) => {
	...
	try {
		console.log(filter, memeId);
		const result = await user.updateOne(filter, {
			$push: { favourites: memeId },
		});
		res.status(200).send({ success: true, msg: "Meme added to favourites!" });
	} catch (error) {
...
router.put("/removeFavourites/:userId", async (req, res) => {
	...
	try {
		console.log(filter, memeId);
		const result = await user.updateOne(filter, {
			$pull: { favourites: memeId },
		});
		res
			.status(200)
			.send({ success: true, msg: "meme removed from favourites" });
	} catch (error) {
		res.status(400).send({ success: false, msg: error });
	}
});
