...
const Home = () => {
	const [
		{ searchTerm, allMemes, memeFilter, filterTerm, gifFilter },
		dispatch,
	] = useStateValue();
	...
	useEffect(() => {
		const filtered = allMemes?.filter((data) => data.artist === memeFilter);
		if (filtered) {
			setFilteredMemes(filtered);
		} else {
			setFilteredMemes(null);
	...
	useEffect(() => {
		const filtered = allMemes?.filter((data) => data.album === gifFilter);
		if (filtered) {
			setFilteredMemes(filtered);
		} else {
			setFilteredMemes(null);
