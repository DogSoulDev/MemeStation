...
export const ImageUploader = ({
	...
	const uploadImage = (e) => {
		...
		const storageRef = ref(
			storage,
			`${isImage ? "Images" : "Gif"}/${Date.now()}-${imageFile.name}`,
		);
		const uploadGif = uploadBytesResumable(storageRef, imageFile);
		uploadGif.on(
			"state_changed",
			(snapshot) => {
			...
			() => {
				getDownloadURL(uploadGif.snapshot.ref).then((downloadUrl) => {
		...
		<label>
			<div className='flex flex-col items-center justify-center h-full'>
				<div className='flex flex-col justify-center items-center cursor-pointer'>
					...
					<p className='text-lg'>
						Click to upload {isImage ? "image" : "gif"}
					</p>
				</div>
			</div>
			<input
...
const DashboardNewGif = () => {
	...
	useEffect(() => {
		if (!gifs) {
			getAllGifs().then((data) => {
				dispatch({ type: actionType.SET_gifs, gifs: data.data });
			});
		}
		if (!allgifs) {
			getAllGifs().then((data) => {
	...
	const deleteImageObject = (gifURL, action) => {
		if (action === "image") {
		...
		const deleteRef = ref(storage, gifURL);
	...
	const saveGif = () => {
		if (!gifImageUrl || !gifImageUrl || !gifName) {
			setSetAlert("error");
			...
			data.then((res) => {
				getAllGifs().then((gifs) => {
					dispatch({ type: actionType.SET_ALL_GIFS, allgifs: gifs.data });
				});
			});
			setSetAlert("Success");
			setAlertMsg("Gif saved successfully!");
			setTimeout(() => {
				setSetAlert(null);
			}, 4000);
			setIsImageLoading(false);
			...
			<div className='grid grid-cols-1 lg:grid-cols-2 gap-4 w-full'>
				<div className='flex flex-col items-center justify-center gap-4'>
					...
					<div className='flex w-full justify-between flex-wrap items-center gap-4'>
						<FilterButtons filterData={gifs} flag={"Meme"} />
						<FilterButtons filterData={allgifs} flag={"Meme"} />
									...
									{!gifImageUrl ? (
										...
										<div className='relative w-full h-full overflow-hidden rounded-md'>
											...
											<button
												...
												onClick={() => {
													deleteImageObject(gifImageUrl, "image");
												}}
											>
...
export default DashboardNewGif;
